...
function checkDistFiles() {
  const sampleDist = path.join(__dirname, 'packages/modules/sample/dist/main.js');
  const cssDist = path.join(__dirname, 'packages/css/dist/index.css');
  
  return fs.existsSync(sampleDist) && fs.existsSync(cssDist);
}
...
async function startDevelopment() {
  log('🚀 Starting DIGIT Development Server...', colors.cyan);
  ...
  try {
    ...
    const devProcess = spawn('npx', [
      'concurrently',
      '--names', 'CSS,Sample,Webpack',
      '--prefix-colors', 'yellow,magenta,cyan',
      '"cd packages/css && npm run start"',
      '"cd packages/modules/sample && npm run build:dev -- --watch"',

...
// Sample variant with admin modules
const enabledModules = ["assignment", "Utilities", "Admin", "Sample"];
...
const MainApp = ({ stateCode, enabledModules }) => {
  ...
  useEffect(() => {
    initLibraries().then(async () => {
      try {
        // Load admin/sample modules if available
        const sampleModule = await import(/* webpackChunkName: "sample" */ "@egovernments/digit-ui-module-sample")
          .catch(() => null);
        
        if (sampleModule?.initSampleComponents) {
          sampleModule.initSampleComponents();
        }
      } catch (error) {
        console.log("Sample modules not available:", error);
      }
      setIsReady(true);
    });
  }, []);

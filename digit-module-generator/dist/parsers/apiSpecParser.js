...
async function parseApiSpec(specPath, entityName) {
  try {
    ...
    if (specPath.startsWith('http://') || specPath.startsWith('https://')) {
      ...
      specContent = specPath.endsWith('.yaml') || specPath.endsWith('.yml') ? yaml.parse(content) : JSON.parse(content);
    }
...
function findEntitySchema(api, entityName) {
  ...
  const variations = [entityName, entityName.toLowerCase(), entityName.toUpperCase(), `${entityName}Request`, `${entityName}Response`, `Create${entityName}Request`, `${entityName}DTO`];

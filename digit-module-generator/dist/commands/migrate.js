...
async function createBackup(modulePath, moduleName) {
  ...
  await fs.copy(modulePath, backupDir, {
    filter: src => {
      ...
      return !relativePath.includes('node_modules') && !relativePath.includes('dist') && !relativePath.includes('.git');
    }
  });
...
async function analyzeModule(modulePath) {
  ...
  // Check for tests
  analysis.hasTests = (await fs.pathExists(path.join(modulePath, '__tests__'))) || (await fs.pathExists(path.join(modulePath, 'src/__tests__')));
...
async function updateImports(dirPath) {
  ...
  for (const file of files) {
    if (file.endsWith('.js') || file.endsWith('.jsx')) {
      ...
      content = content.replace(/@egovernments\/digit-ui-react-components/g, '@egovernments/digit-ui-components');
